...
```bash
git clone https://github.com/Taranjeet07/InternGuard-.git
cd InternGuard-
```
...
Create the required `.env` file(s) and configure the database and API credentials used by the project.
...
> Use the actual variable names required by the project. Never commit API keys or other secrets to GitHub.

### Run the Application

Start the backend and frontend separately using the scripts defined in their respective `package.json` files.
...
- **Demo Video:** Add your video demonstration link here.
...
Built as a team project for the hackathon.

Responsibilities included frontend development, backend/API development, AI analysis, security and URL analysis, database integration, deployment, and testing.
...
InternGuard provides risk indicators and verification guidance. A high-risk result does not by itself prove that a company, recruiter, or opportunity is fraudulent.

Users should independently verify important opportunities through official company or institutional sources.
